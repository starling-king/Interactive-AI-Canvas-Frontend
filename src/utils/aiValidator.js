...
export const healAiPayload = (rawPayload) => {
    try {
        ...
        const sanitizedEdges = edges.reduce((acc, edge) => {
            ...
            // THE FIX: Translate AI's generic direction into the specific double-ID syntax
            const rawSource = edge.sourceHandle || 'bottom';
            const rawTarget = edge.targetHandle || 'top';

            // If the AI accidentally includes the suffix, don't double it
            const safeSource = rawSource.includes('-source') ? rawSource : `${rawSource}-source`;
            const safeTarget = rawTarget.includes('-target') ? rawTarget : `${rawTarget}-target`;
